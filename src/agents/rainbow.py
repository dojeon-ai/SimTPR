...
class RAINBOW(BaseAgent):
    ...
    def _update(self):
        ...
        with torch.no_grad():
            ...
            #offset = torch.linspace(0, ((self.cfg.batch_size - 1) * self.num_atoms), 
            #        self.cfg.batch_size).unsqueeze(1).expand(self.cfg.batch_size, self.num_atoms).to(l)
            #m.view(-1).index_add_(0, (l + offset).view(-1), (target_q_dist * (u.float() - b)).view(-1))  # m_l = m_l + p(s_t+n, a*)(u - b)
            #m.view(-1).index_add_(0, (u + offset).view(-1), (target_q_dist * (b - l.float())).view(-1))  # m_u = m_u + p(s_t+n, a*)(b - l)
    ...
    def train(self):
        ...
        for t in tqdm.tqdm(range(self.cfg.num_timesteps)):
